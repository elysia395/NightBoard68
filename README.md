...
由社区协作者 [@TeaClearInkII](https://github.com/TeaClearInkII) 贡献的稳定性修复批次
（PR #12 / #13，详尽记录见仓库 [FIXLOG.md](FIXLOG.md)）。**电脑端 Agent 必须同步更新。**

### 蓝牙：切后台回来第一次按键卡键连发
- 根因：断链窗口里 keydown 送达、keyup 丢失，电脑端永久收不到抬起
- 修复：发送失败/连接未就绪置 dirty 标记，重连建立即补发当前状态（无按键则发
  空报告解除卡键）；链路被静默挂起时（3 秒无成功发送）在下次按键前先对齐一次

### 局域网：方向键输出 8/2/4/6（NumLock 开时）
- 根因：Agent 方向键走扫描码 + E0 扩展前缀注入，部分环境前缀不生效，
  方向键被 Windows 认成小键盘 8/2/4/6
- 修复：编辑键区（Insert/Home/PgUp/Delete/End/PgDn/方向键）全部改走
  VK 注入，与 NumLock、键盘布局无关

### 局域网：反复误判断连 + 触控板只能移动、点击失效
- 根因：心跳与输入共用一个队列，触控板刷屏打满队列导致 ping 丢失 → 误判断线
  循环；重连窗口内点击全部被丢弃；新旧连接心跳线程互相干扰
- 修复：心跳直写 socket 不再与输入抢队列（输入队列 512→2048）；
  连接代号机制让旧线程自灭

### 其他修复
- 局域网鼠标**中键**支持（Agent 此前只处理左/右键）
- NumLock / ScrollLock 状态变化也回传 LED（此前只有 CapsLock），
  小键盘「修改模式」键帽整体切换（NumLock 关 → 7 变 Home、8 变 ↑ …，主机权威）
- 大写锁定键帽点亮样式与其他锁存键统一
- Agent 启动时检测管理员权限并向管理员窗口注入失败（Windows UIPI）时给出明确提示：
  以管理员身份运行 Agent 即可控制管理员权限的程序

## v1.4.0 改动点（相对 v1.3.2）

由社区协作者 [@TeaClearInkII](https://github.com/TeaClearInkII) 贡献的大版本更新（PR #10）。

### 新增：数字小键盘
- **真实电脑小键盘布局**：标准 4×5 配列（含跨行 + / Ent、跨 2 列 0），悬浮窗
  右下角把手拖拽缩放，位置与大小持久化
- **真小键盘 HID 键码**（Keypad 页专用码），电脑端识别为数字小键盘而非主键盘区
- **NumLock 关闭操作标注**：数字键右上角小字标注通用映射（7→Home…0→Ins）；
  收到主机 LED 确认 NumLock 关 → 橙色高亮，未知状态 → 灰色常驻，不猜测显示

### 新增：LED 状态（主机权威）
- CapsLock / NumLock 状态全部来自电脑回传（蓝牙 Output Report / 局域网 led 消息
  + 心跳兜底）；未收到 LED 时不显示锁定高亮，不做本地猜测

### 新增：软键盘（中文 / 任意字符直接打进电脑）
- 顶部「软键盘」唤起系统输入法（无输入框），确认的文字逐字直接发送——
  中文、emoji 都能打
- **需局域网模式 + 新版 `NightBoardAgent`**（新增 txt 协议 + Unicode 注入）；
  蓝牙模式下自动隐藏入口
- 拼音未上屏的合成区内容不会误发；退格自动同步、粘贴整段发送

### 触控板
- **鼠标键列按住不松 + 触控板移动 = 拖放 / 框选**（快速点按仍是普通点击）
- 悬浮触控板大小可调（缩放把手，110~460dp），位置与大小持久化
- 修复双指手势时触控板移动事件丢失

### 组合键与弹窗
- 组合按钮流式排列、宽度自适应；自定义组合可新增 / 长按删除
- 竖屏式编辑器支持**多主键组合**（同时按下的多行键值，自动去重），
  可选键值扩展到小键盘
- 新的统一深色圆角弹窗框架（右上角 ✕、超高内容整体滚动、
  长按选项改为底部白色圆角选项条）

### 主界面 / 设置 / 键位
- 主界面与设置卡片式改版：SVG 图标、已配对设备按类型显示图标、
  当前连接高亮；首次进入默认键盘模式
- 蓝牙目标切换（switchHost）：切换已配对电脑时先断旧再连新
- Fn 层调整；**Mac 键位 ⌃⌥⌘ 标签**（设置页「键盘布局」切换）；
  竖屏页沉浸式 / 屏幕常亮与横屏对齐
- 长按引导：首次进键盘页弹一次「长按小技巧」，长按过的按钮记忆标记

## v1.3.2 改动点（相对 v1.3.1）

### 修复：修饰键与组合键触发
- **长按 Alt + 连点 Tab 可循环切换所有后台窗口**：修饰键手指仍按住时，
  组合跨多次按键保持、抬手才提交（此前第一个键按下后锁存即被消费，
  第二个 Tab 就不再带 Alt，只能在两个窗口间切换）
- 按住 Tab 不松手由电脑端连发持续循环切窗（蓝牙 / 局域网均恢复可用）
- 横屏 Fn 点按锁存恢复可用（此前抬 Fn 手指会立即清掉锁存，点按等于白按）；
  按住 Fn 打字后抬指即结束 Fn 层；抬 Fn 手指不再误清已锁存的修饰键
- 竖屏：按住 Alt 打组合键不再在 550ms 后被「换位对话框」劫持
  （其他按键 / 触摸板开始使用时自动取消换位判定）
- 大写锁定（双击 Shift）时点「中英 / 输入法」按钮不再悄悄解除锁定

### 蓝牙连接稳定性
- **断连后手机侧自动回连**（3s/8s/20s/40s 递增间隔；此前只能靠 Windows
  自动回连或手动点「检查」）
- **蓝牙通道鼠标报告 10ms 合并**：修复 v1.3.1 快速拖动节流旁路带来的
  报告洪峰（120Hz+ 逐条直发 HID 中断通道），合并发送完全无损，
  慢速拖动手感不变；局域网模式不受影响
- 主页新增「申请电池优化豁免」：防止部分 ROM 杀后台导致 HID 注册重建
  （用户感知即频繁断连）
- 设置页新增「连接日志」：最近 30 条蓝牙连接事件（连接 / 断开 / 回连尝试），
  断连发生在闲置时还是使用中一目了然

### 其他
- 新增 `CONTRIBUTING.md` 贡献指南；`NightBoard68-mac` 分支恢复 PR 通道
  （与 main 无差异的分支 GitHub 拒绝建 PR，已补初始化提交）

## v1.3.1 改动点（相对 v1.3.0）

### 新增：按键宽度自定义（PR #7，来自社区协作者 [@pjh-bit](https://github.com/pjh-bit)）
- 横屏 68 键与竖屏模式都支持**选中任意按键、水平拉宽/缩窄**（进入布局编辑态，
  实时预览重排）
- 键宽按横竖屏**各自独立记忆**，提供 Done 确认与 Reset 一键恢复默认

### 新增：触摸板手势与交互（横竖屏共用重构后的 TouchpadEngine）
- **甩动惯性滚动**：轻甩一下滚多屏，新手势或抬指即停
- **轻点-轻点-按住 = 拖选**：选中文字 / 文件不再需要精确长按
- **双指捏合 = Ctrl+滚轮缩放**：捏合与滚动按「符号化意图竞速」判定，
  斜向滚动不会误触缩放；张开=放大
- 竖屏快捷键区 Ctrl / Alt / Tab / Win / Esc 支持**长按互换位置**，顺序持久化
- **快速拖动降延迟**：累计 3px 即绕过 12ms 刷新节流，快滑更跟手

### 修复
- **触摸板滚动永久失效的根因**：滚轮事件被光标移动的亚像素残量「饿死」——
  动过光标之后滚动就不再响应（上游遗留）
- 双指滚动后**光标冻结**、拖选失灵、捏合永不锁定（系统复用指针 id 导致的残留状态）
- 双指滚动进行中**误触发捏合缩放**（绝对累积改为符号化竞速判定）
- 局域网模式**重连后轻点失灵**：电脑端代理在新连接建立时归零鼠标按键基线
- 断线瞬间电脑端**鼠标按键卡死**：连接断开时代理补发鼠标抬起

### 调整
- 双指滚动灵敏度（含惯性）整体减半；捏合方向调整为张开=放大
